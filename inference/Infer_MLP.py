"""SimpleMLP inference for saved x_new data and a trained checkpoint."""
...
def load_x_new(data_dir: Path) -> torch.Tensor:
    ...
    if a.ndim == 2:
        if a.shape[0] == 1 or a.shape[1] == 1:
            return torch.from_numpy(a.reshape(-1))
        # Keep as 2D; orientation will be resolved in window builder.
...
def build_inference_windows(x: torch.Tensor, L: int, out_len: int) -> torch.Tensor:
    ...
    if x.dim() == 2:
        # Resolve orientation: (B,N) preferred; accept (N,B) if first dim matches N.
        if x.shape[1] == N:
            xb = x
        elif x.shape[0] == N:
            xb = x.t()
        ...
        B = xb.shape[0]
        n = torch.arange(0, out_len, dtype=torch.long, device=x.device)
        cols = [xb[:, n + L - lag] for lag in range(L + 1)]  # each (B,out_len)
        Xb = torch.stack(cols, dim=2)  # (B,out_len,L+1)
        return Xb.reshape(B * out_len, L + 1)  # feed to MLP as a big batch
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="Infer SimpleMLP from x_new to y_predict (1024)")
    p.add_argument("--data-dir", type=Path, default=root, help="folder with x_new.npy or x_new.mat")
    p.add_argument(
        "--pretrained",
        type=Path,
        default=root / "Final_simpleMLP.pt",
        help="pretrained weights .pt (dict with keys: model, L, hidden, num_hidden_layers)",
    )
    p.add_argument("--out-len", type=int, default=1024, help="length of y_predict")
    p.add_argument("--out-npy", type=Path, default=root / "y_predict_MLP.npy")
    ...
    model = SimpleMLP(L=L, hidden=hidden, num_hidden_layers=num_hidden_layers)
    ...
    # If input was a batch, X was flattened to (B*out_len, L+1) and y_hat is (B*out_len,).
    ...
    if args.out_mat is not None:
        savemat(args.out_mat, {"y_predict": y_hat.reshape(-1, 1)})
