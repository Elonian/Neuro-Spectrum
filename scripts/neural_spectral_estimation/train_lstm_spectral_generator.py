"""Train the supplied SimpleLSTM model as a learned spectral process generator."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train SimpleLSTM on lagged x/y data")
    ...
    logger = setup_logger("train_lstm_spectral_generator")
    ...
    model = build_model("lstm", lag=lag, config=model_config)
    logger.info("Training LSTM lag=%d data_dir=%s device=%s", lag, data_dir, device)
    losses = train_model(
        model,
        loader,
        device=device,
    ...
    checkpoint_path = model_dir / f"simple_lstm_lag_{lag}.pt"
    save_checkpoint(
        checkpoint_path,
        checkpoint_payload("lstm", model, lag=lag, model_config=model_config, losses=losses),
    )
    loss_path = model_dir / f"simple_lstm_lag_{lag}_losses.npy"
    save_array(loss_path, np.asarray(losses, dtype=np.float32))
    save_json(
        model_dir / f"simple_lstm_lag_{lag}_summary.json",
        {
            "model": "lstm",
            "lag": lag,
    ...
    logger.info("Saved LSTM checkpoint to %s", checkpoint_path)

    try:
        plot_training_loss(
            resolve_project_path(
                Path(config["paths"]["figures_dir"]) / f"simple_lstm_lag_{lag}_training_loss.png",
                root=PROJECT_ROOT,
            ),
            losses=losses,
            title=f"SimpleLSTM Training Loss, lag={lag}",
        )
